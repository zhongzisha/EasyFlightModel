...
float UeasyFM::BlendInput(float low, float high) const {
	return FMath::Lerp(FMath::Clamp(low, -1.0f, 1.0f), FMath::Sign(high), FMath::Min(FMath::Abs(high), 1.0f));
};

float UeasyFM::Limiter(float input, float limitedVar, float min, float max, float sensitivity) const {
	return FMath::Clamp(input + (limitedVar - FMath::Clamp(limitedVar, min, max))*sensitivity, -1.0f, 1.0f);
};

float UeasyFM::Transit(float input, float target, float upspeed, float downspeed, float DeltaTime) const {
