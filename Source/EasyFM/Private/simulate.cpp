...
void UeasyFM::Simulate(FBodyInstance* bodyInst, float DeltaTime, bool apply, bool debug) {
	...
	//engine
	float thrustTemp = 0.0f;
	FVector enginevec = fwdvec;
	FVector WashVelocity = FVector (0,0,0);
	if (EngineEnabled) {

		if (VTOLEnabled) {
			enginevec = enginevec.RotateAngleAxis(-VTOLPos*VTOLMaxAngle, rightvec);
		};

		thrustTemp = EnginePower(TotalSpeed,Mach,AirDensity);

		if (PropWash) {
			WashVelocity = (enginevec*(thrustTemp*PropWashVelocity));
			...
			FVector washstabtorque = Stability(Transform, Velocity + (enginevec*WashVelocity), AirDensity, SpeedOfSound);
	...
	if (LiftAsymmetryEnabled && (AsymmetrySampleCount > 1) && (LiftAsymmetryPortion>0.0f)) {
		...
		FVector Lwind= GetWind(Transform.GetLocation());
	...
	//RCS
	FVector rcs=RCSTorque(thrustTemp);
	...
	FVector finalForce = ((AeroForcesRotated*AirDensity) + FVector(0.0f, 0.0f, AirDensity*Buoyancy) + (enginevec*thrustTemp*EngineFullPower));
	...
	FVector finalTorque = (torqueLoc + 
		(fwdvec*thrustTemp*PropTorqueRoll) + 
		(upvec*thrustTemp*PropTorqueYaw) + 
		(rightvec*thrustTemp*PropTorquePitch)
		);
